...
void show_interface()
{
	...
	initgraph(SCREEN_WIDTH, SCREEN_HEIGHT);//根据背景图片的长和宽初始化窗口，使之铺满窗
	setbkcolor(BLUE);
	putimage(0,0,&background_img);//在窗口上绘制背景图片
...
{
	int i;
	//控制按钮坐标数组(0-4依次是增加、存状态、删除、退出、导入按钮的坐标)
	...
	char control_button_text[5][8] = {"增加", "存状态", "删除", "退出", "导入"};
	//绘制增加按钮
	for(i = 0; i < 5; i++){
